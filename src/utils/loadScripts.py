...
def setLoggingFormat(settingsDict):
    ...
    base_fmt = (
        ...
        if settingsDict.get("VERBOSE", False)
    ...
    # Clear existing handlers and set up the new configuration
    logging.basicConfig(level=log_level_num, force=True)
...
def showSettings(settingsDict):
    ...
    for instance in settingsDict["INSTANCES"]:
        if settingsDict[instance + "_URL"]:
            logger.info(
                "%s: %s",
                settingsDict[instance + "_NAME"],
                settingsDict[instance + "_URL"],
            )

    if settingsDict["QBITTORRENT_URL"]:
        logger.info("qBittorrent: %s", settingsDict["QBITTORRENT_URL"])
...
async def instanceChecks(settingsDict):
            ...
            if not error_occured:
                ...
                if settingsDict[instance + "_MIN_VERSION"]:
                    if version.parse(current_version) < version.parse(
                        ...
                        error_occured = True
                        logger.error(
                            "!! %s Error: !!", settingsDict[instance + "_NAME"]
                        )
                        logger.error(
    ...
    if error_occured:
        logger.warning(
            "At least one instance was not reachable. Waiting for 60 seconds, then exiting Decluttarr."
        )
