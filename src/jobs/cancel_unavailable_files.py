...
async def cancel_unavailable_files(
    settingsDict,
    BASE_URL,
    API_KEY,
    NAME,
    ...
    try:
        ...
        qbitHashes = list(
            set(queueItem["downloadId"].upper() for queueItem in queue["records"])
        )
        ...
        for qbitItem in qbitItems:
            if "state" in qbitItem and "availability" in qbitItem:
                if qbitItem["state"] == "downloading" and qbitItem["availability"] < 1:
                    ...
                    qbitItemFiles = await rest_get(
                        settingsDict["QBITTORRENT_URL"] + "/torrents/files",
                    ...
                    for qbitItemFile in qbitItemFiles:
                        if all(
                            ...
                            if (
                                qbitItemFile["availability"] < 1
                                and qbitItemFile["progress"] < 1
                                and qbitItemFile["priority"] != 0
                            ):
                                logger.verbose(
                                    ">>>>> %s", qbitItemFile["name"].split("/")[-1]
                                )
                                if not settingsDict["TEST_RUN"]:
                                    await rest_post(
                                        url=settingsDict["QBITTORRENT_URL"]
                                        + "/torrents/filePrio",
                                        data={
                                            "hash": qbitItem["hash"].lower(),
                                            "id": qbitItemFile["index"],
                                            "priority": 0,
                                        },
                                        cookies=settingsDict["QBIT_COOKIE"],
                                    )
